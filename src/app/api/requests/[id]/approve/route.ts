...
export async function POST(
  ...
  db.run(
    "UPDATE watch_requests SET status = 'approved', resolved_at = unixepoch() WHERE id = ?",
    [id]
  );

  if (request.telegram_message_id) {
    await editApprovalMessage(request.telegram_message_id, true, request.kid_name, request.video_title);
  }
